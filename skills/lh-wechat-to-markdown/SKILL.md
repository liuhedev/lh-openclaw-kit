...
- 适用于用户可正常访问的微信公众号文章页面
- 支持抓取正文、标题、作者、发布时间，并保存为 Markdown + HTML 快照
- 对公开文章通常可直接无头抓取；遇到登录、懒加载或访问限制场景，可切换为有头模式辅助完成
- 不同文章模板、图片加载方式和页面结构可能存在差异，个别场景建议结合保存下来的 HTML 快照进一步处理
